...
static uint8_t SPI_RW(uint8_t value) {
	register uint8_t i;
	for (i = 0; i < 8; i++) {
		//	for(delay=0;delay<1;delay++); ////////////////////////////delay/////////////////////////////
		if (value & 0x80) {
...
/***********************************************************************
 * Function: Interrupt EXTI1() , obsluga przerwania od IRQ
 ***********************************************************************/
#if RF70_UseIRQ == 1
...
void RFM70_Init() {
...
#if RF70_UseIRQ == 1
	EXTI0_Config(); // uruchamiam przerwania
#endif
	...
	GPIOA->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR8 | GPIO_OSPEEDER_OSPEEDR9
			| GPIO_OSPEEDER_OSPEEDR10 | GPIO_OSPEEDER_OSPEEDR12; // 50 MHz
	///GPIOA->PUPDR |= GPIO_PUPDR_PUPDR11_0;// | GPIO_PUPDR_PUPDR13_0; // MISO i IRQ input HI-Z do sprawdzenia !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	GPIOA->ODR |= CSN; // CSN=1
	GPIOA->ODR &= ~(MOSI | CE | SCK); // MOSI,CE,SCK=0
// Init soft SPI end ****************
...
void EXTI0_Config() {
	...
	SYSCFG->EXTICR[0] |= SYSCFG_EXTICR1_EXTI0_PB; // Exti in input PB0 moze byc tez
